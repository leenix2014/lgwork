...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            // Controller允许访问的用户
            String[] controllerUsers = {};
            Security sec = aClass.getAnnotation(Security.class);
            if (sec != null) {
                controllerUsers = sec.values();
            }
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                // Method允许访问的用户
                String[] methodUsers = {};
                Security s = method.getAnnotation(Security.class);
                if (s != null) {
                    methodUsers = s.values();
                }
                String[] allowedUser = interact(controllerUsers, methodUsers);

                // 把method所有信息及url封装为一个Handler
                Handler handler = new Handler(entry.getValue(),method, Pattern.compile(url), allowedUser);
    ...
    private String[] interact(String[] controllerUsers, String[] methodUsers) {
        if (controllerUsers.length == 0) {
            return methodUsers;
        }
        if (methodUsers.length == 0){
            return controllerUsers;
        }
        return innerJoin(controllerUsers, methodUsers);
    }

    private String[] innerJoin(String[] controllerUsers, String[] methodUsers) {
        List<String> controllerList = Arrays.asList(controllerUsers);
        List<String> methodList = Arrays.asList(methodUsers);
        List<String> common = new ArrayList<>();
        for(String controllerUser : controllerList){
            if (controllerUser == null){
                continue;
            }
            if (methodList.contains(controllerUser)) {
                common.add(controllerUser);
            }
        }
        String[] result={};
    ...
    private boolean contains(String[] allowedUsers, String username) {
...
//        method.invoke() //
        ...
        String[] allowedUsers = handler.getAllowedUsers();
        boolean canAccess = contains(allowedUsers, username);
        if (!canAccess){
            resp.getWriter().write("You are not allowed to see this page");
            return;
        }
